...
export default function StatsGrid() {
    ...
    const stats = {
        ...
        myTasks: allTasks.filter(t => t.assigneeId === user?.id).length,
        overdueIssues: allTasks.filter(t => t.dueDate && new Date(t.dueDate) < now && t.status !== 'Done').length,
    };

    const statCards = [
        { icon: FolderOpen, title: "Total Projects", value: stats.totalProjects, subtitle: "all projects", bgColor: "bg-blue-500/10", textColor: "text-blue-500" },
        { icon: CheckCircle, title: "Active Projects", value: stats.activeProjects, subtitle: "currently active", bgColor: "bg-emerald-500/10", textColor: "text-emerald-500" },
